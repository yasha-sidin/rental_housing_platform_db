...
В локальном стенде отдельный восстановительный узел представлен сервисом `recovery-node` в Docker Compose. Он запускается только по требованию через профиль `tools`, не участвует в Patroni и не принимает клиентский трафик рабочего контура. Данные восстанавливаются в отдельный volume `recovery_node_data`, поэтому проверка PITR не перезаписывает рабочие PostgreSQL-узлы.
