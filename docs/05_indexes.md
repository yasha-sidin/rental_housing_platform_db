...
используется для фильтрации `bookings.created_by_user_id`. В плане используется один
составной индекс `idx_bookings_created_by_user_creation_date_desc`, а не два отдельных
индекса. Но в этом конкретном плане PostgreSQL применяет его прежде всего по первой
части ключа: `created_by_user_id`.

В этом запуске PostgreSQL выбрал `Bitmap Index Scan` и отдельную сортировку по
`creation_date DESC`. `Bitmap Index Scan` не сохраняет порядок строк из индекса:
сначала собирается bitmap строк-кандидатов, затем строки читаются из таблицы. Поэтому
вторая часть составного индекса, `creation_date DESC`, здесь не используется для
готовой выдачи в нужном порядке, и сортировка выполняется отдельным узлом `Sort`.

Вывод: составной индекс используется, но в этом плане раскрывается не весь его
потенциал. Он ускоряет фильтрацию по пользователю через первую колонку индекса. На
другом объеме данных или другой селективности PostgreSQL может выбрать прямой
`Index Scan` по этому же индексу и тогда использовать порядок `creation_date DESC`
из второй части ключа без отдельной сортировки.
