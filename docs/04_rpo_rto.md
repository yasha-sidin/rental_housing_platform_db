...
Для логических ошибок нужен PITR: восстановление отдельного узла из full backup и WAL archive на момент до ошибки.

## RTO

`RTO` - время восстановления работы после аварии.

В проекте измеряются три типа RTO:

- автоматическое переключение primary;
...
2. Дождаться, пока маршрут записи одного из HAProxy начнет вести на новый primary.
