...
Подробный список доменных правил описан отдельно в `docs/10_domain_invariants.md`. Это разделение сделано намеренно: текущий документ объясняет устройство схемы и миграций, а документ с инвариантами показывает, какие бизнес-ограничения защищаются базой данных.
