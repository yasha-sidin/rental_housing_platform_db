...
$$
    ...
    BEGIN
        ...
        DELETE
        FROM reviews
        WHERE booking_id IN (SELECT b.id
                             FROM bookings b
        ...
        DELETE
        FROM bookings
        WHERE created_by_user_id IN (SELECT id
                                     FROM users
                                     WHERE username LIKE 'seed_guest_%');
        ...
        -- ========================================================================
        SELECT id
        INTO v_day_completed_1
        FROM listing_availability_days
        WHERE listing_id = v_listing_ny
        ...
        SELECT id
        INTO v_day_completed_2
        FROM listing_availability_days
        WHERE listing_id = v_listing_paris
          AND status = 'available'
        ...
        WHERE listing_id = v_listing_ny
          AND status = 'available'
          AND available_date >= current_date + 3
        ORDER BY available_date
        LIMIT 1;
        ...
        INTO v_day_created
        FROM listing_availability_days
        WHERE listing_id = v_listing_sf
          AND status = 'available'
          AND available_date >= current_date + 4
        ORDER BY available_date
        LIMIT 1;

        SELECT id
        INTO v_day_confirmed
        FROM listing_availability_days
        WHERE listing_id = v_listing_berlin
          AND status = 'available'
          AND available_date >= current_date + 5
        ORDER BY available_date
        LIMIT 1;

        SELECT id
        INTO v_day_cancelled_guest
        FROM listing_availability_days
        WHERE listing_id = v_listing_london
          AND status = 'available'
          AND available_date >= current_date + 6
        ORDER BY available_date
        LIMIT 1;

        SELECT id
        INTO v_day_cancelled_owner
        FROM listing_availability_days
        WHERE listing_id = v_listing_dubai
          AND status = 'available'
          AND available_date >= current_date + 7
        ORDER BY available_date
        LIMIT 1;
        ...
        INTO v_day_expired
        FROM listing_availability_days
        WHERE listing_id = v_listing_barcelona
          AND status = 'available'
          AND available_date >= current_date + 8
        ORDER BY available_date
        LIMIT 1;

        SELECT id
        INTO v_day_failed_payment
        FROM listing_availability_days
        WHERE listing_id = v_listing_berlin
          AND status = 'available'
          AND available_date >= current_date + 9
        ORDER BY available_date
        LIMIT 1;
        ...
        VALUES (v_booking_completed_1, v_usd_currency, 18000, 'paid', 'seed-paid-completed-1',
        ...
        INSERT INTO payments (booking_id, currency_id, amount_in_minor, status, provider_payment_session_id,
                              provider_payment_session_expires_at)
        VALUES (v_booking_completed_2, v_eur_currency, 20000, 'paid', 'seed-paid-completed-2',
                now() + interval '5 minutes');
        ...
        INSERT INTO payments (booking_id, currency_id, amount_in_minor, status, provider_payment_session_id,
                              provider_payment_session_expires_at)
        VALUES (v_booking_pending, v_usd_currency, 17000, 'initiated', 'seed-initiated-pending-1',
                now() + interval '5 minutes');
        ...
        INSERT INTO payments (booking_id, currency_id, amount_in_minor, status, provider_payment_session_id,
                              provider_payment_session_expires_at)
        VALUES (v_booking_confirmed, v_eur_currency, 24000, 'paid', 'seed-paid-confirmed-1',
                now() + interval '5 minutes');
        ...
        INSERT INTO payments (booking_id, currency_id, amount_in_minor, status, provider_payment_session_id,
                              provider_payment_session_expires_at)
        VALUES (v_booking_cancel_guest, v_gbp_currency, 25000, 'cancelled', 'seed-cancelled-payment-1',
                now() + interval '5 minutes');
        ...
        INSERT INTO payments (booking_id, currency_id, amount_in_minor, refunded_amount_in_minor, status,
                              provider_payment_session_id, provider_payment_session_expires_at)
        VALUES (v_booking_cancel_owner, v_aed_currency, 76000, 76000, 'refunded', 'seed-refunded-owner-cancel-1',
                now() + interval '5 minutes');
        ...
        INSERT INTO payments (booking_id, currency_id, amount_in_minor, status, provider_payment_session_id,
                              provider_payment_session_expires_at)
        VALUES (v_booking_expired, v_eur_currency, 17000, 'expired', 'seed-expired-payment-1',
                now() + interval '5 minutes');
        ...
        INSERT INTO payments (booking_id, currency_id, amount_in_minor, status, provider_payment_session_id,
                              provider_payment_session_expires_at)
        VALUES (v_booking_failed, v_eur_currency, 23000, 'failed', 'seed-failed-payment-1',
                now() + interval '5 minutes');
        ...
        IF v_day_cancelled_owner IS NOT NULL THEN
            ...
            INSERT INTO payments (booking_id, currency_id, amount_in_minor, refunded_amount_in_minor, status,
                                  provider_payment_session_id, provider_payment_session_expires_at)
            VALUES (v_booking_cancel_owner, v_gbp_currency, 24000, 7000, 'partially_refunded', 'seed-partial-refund-1',
                    now() + interval '5 minutes');
        END IF;
