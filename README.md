...
Подробная модель предметной области описана в `docs/01_domain_model.md`, а отдельный список бизнес-инвариантов - в `docs/09_domain_invariants.md`.
...
Такой подход разделяет хранение данных, обслуживание кластера и процесс резервного копирования. В локальном стенде резервные данные сохраняются в MinIO, чтобы сценарии backup и PITR были воспроизводимыми без внешней инфраструктуры. В production этот компонент заменяется надежным внешним S3-совместимым хранилищем.
...
- `docs/03_cluster_topology.md` - устройство кластера: Patroni, etcd, PgBouncer, HAProxy и Percona Monitoring and Management.
...
- `docs/09_domain_invariants.md` - доменные инварианты и границы ответственности базы данных.
